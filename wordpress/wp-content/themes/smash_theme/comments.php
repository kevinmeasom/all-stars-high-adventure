...
<div id="comments" class="comments-area">
    ...
    $comments_args = array(
		...
		'class_submit' => __('btn btn-secondary', 'smash_theme'),
        // Change the title of send button 
        'label_submit' => __( 'Submit', 'smash_theme' ),
        // Change the title of the reply section
        'title_reply' => __( '', 'smash_theme' ),
	...
	if ( have_comments() ) :
		...
		<h2 class="comments-title">
			<?php
			$smash_theme_comment_count = get_comments_number();
			if ( '1' === $smash_theme_comment_count ) {
				printf(
					/* translators: 1: title. */
					esc_html__( 'One thought on &ldquo;%1$s&rdquo;', 'smash_theme' ),
					'<span>' . get_the_title() . '</span>'
				);
			} else {
				printf( // WPCS: XSS OK.
					/* translators: 1: comment count number, 2: title. */
					esc_html( _nx( '%1$s thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', $smash_theme_comment_count, 'comments title', 'smash_theme' ) ),
					number_format_i18n( $smash_theme_comment_count ),
					'<span>' . get_the_title() . '</span>'
				);
			}
			?>
		...
		if ( ! comments_open() ) :
			?>
			<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'smash_theme' ); ?></p>
			<?php
		endif;
